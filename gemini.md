...
*   **Focus-First UI Redesign (Control Center):**
    *   **Navbar Decluttering:** Убрал второстепенные фильтры в сайдбар, оставив хедер чистым.
    *   **Unified Sidebar (Control Center):** Все настройки (Язык, Тема, Уровень) теперь в одном месте.
    *   **Smart Randomizer:** Вернул кнопку рандомайзера в хедер (рядом с меню) для быстрого доступа, так как это часто используемое действие.
    *   **UI Polish:** Оптимизировал отступы и размеры элементов в новом компактном хедере.
